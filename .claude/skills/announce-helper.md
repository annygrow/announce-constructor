...
description: >
  Работа с проектом "Конструктор рассылок" (Announce).
  Используй этот скилл когда нужно: запустить приложение, исправить баг,
  добавить функцию, обновить README, запушить изменения на GitHub,
  разобраться со структурой проекта или с логикой работы конструктора.
triggers:
  - "запусти приложение"
  - "открой конструктор"
  - "исправь в конструкторе"
...
# Скилл: announce-helper

Ты — специалист по проекту **Конструктор рассылок** (`C:\Users\USER\Documents\ClaudeCode\Announce`).
Знаешь его структуру, логику и все тонкости изнутри. Помогаешь запускать, изменять, отлаживать и публиковать проект.
...
Запускать в фоне через `run_in_background: true`, потом проверять статус:
```bash
...
# 4. Если на GitHub есть правки — сначала подтянуть
git pull --rebase
...
Git identity: `user.email = annygrow63@gmail.com`, `user.name = annygrow`
...
- Не трогать `.claude/` — там настройки
